...
void functions::check_katastash(){
  for(i=0;i<oxhm;i++){
    ...
    if(xarths[a][b]==1){
          ...
          thesi_y[i]=SIZ_Y-1;
          if(ID[i]>=2 && ID[i]<=4){
            snprintf(buf, sizeof(buf), "To %s pou htan sto %d,%d kahke kai metaferthike sto kentro", en[i]->get_name().c_str(), a ,b);
      //Render the text
            message1 = TTF_RenderText_Solid( font, buf, textColor );
      //Apply the images to the screen
            FillRect(0,520,660,20,0x0);
            apply_surface( 0, 520, message1, screen );
            en[i]->set_katastash(0);
            
          }
          else{
            snprintf(buf, sizeof(buf), "To %s pou htan sto %d,%d kahke kai metaferthike sto kentro", ed[i]->get_name().c_str(), a ,b);
      //Render the text
            message1 = TTF_RenderText_Solid( font, buf, textColor );
      //Apply the images to the screen
            FillRect(0,520,660,20,0x0);
    ...
    if(ID[i]>=2 && ID[i]<=4){
      if(en[i]->get_nero()==0 || en[i]->get_kausima()==0){
        ...
        snprintf(buf, sizeof(buf), "To %s metaferthike sto kentro", en[i]->get_name().c_str());
      //Render the text
        message1 = TTF_RenderText_Solid( font, buf, textColor );
      //Apply the images to the screen
        FillRect(0,520,660,20,0x0);
  ...
  if(ID[i]>=5 && ID[i]<=7){
    if(ed[i]->get_nero()==0 || ed[i]->get_kausima()==0){
      ...
      if(ed[i]->get_id()==5){
        ...
        snprintf(buf, sizeof(buf), "To %s metaferthike sto kentro", ed[i]->get_name().c_str());
      //Render the text
        message1 = TTF_RenderText_Solid( font, buf, textColor );
      //Apply the images to the screen
        FillRect(0,520,660,20,0x0);
